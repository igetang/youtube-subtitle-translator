...
if (USE_ABORT_CONTROLLER_ARCHITECTURE) {
  ...
  console.debug('[debug][service-worker] ✓ AbortTimeoutManager已初始化');
}
      ...
      port.onDisconnect.addListener(async () => {
        ...
        try {
          ...
          if (associatedTabId) {
            ...
            // 注释掉操作确认日志
            // console.debug(`[debug][service-worker] 已通知标签页${associatedTabId}更新UI`);
          } else {
            // 静默处理无关联标签页
            // console.warn('[service-worker] 无关联标签页ID，无法通知UI更新');
          }
          // 保留一条简洁的关闭日志
          console.debug('[debug][service-worker] ✓ Popup关闭');
...
chrome.runtime.onStartup.addListener(async () => {
  ...
  try {
    ...
    // 静默处理启动状态
    // console.debug('[debug][service-worker] ✓ onStartup: Popup状态已禁用');
  } catch (error) {
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  // 简化消息日志，只在错误时输出
  // console.debug(`[debug][service-worker] 收到消息: ${message.type || message.action}`);
    ...
    getPopupState().then(isCurrentlyOpen => {
      ...
      if (isCurrentlyOpen) {
        ...
        chrome.runtime.sendMessage({
          ...
          // 简化成功日志
          // console.debug(`[debug][service-worker] ✓ closePopup`);
        ...
        chrome.action.setPopup({
          ...
          chrome.action.openPopup().then(() => {
            // 简化成功日志
            // console.debug(`[debug][service-worker] ✓ openPopup`);
            ...
            chrome.windows.create({
              ...
              if (chrome.runtime.lastError) {
                ...
                // 简化备用方案日志
                // console.debug(`[debug][service-worker] ✓ openPopup (备用方案)`);
  ...
  (async () => {
    try {
      ...
      if (response && response.success === true && response.status) {
        // 简化响应日志，只在错误时输出
        // console.debug(`[debug][service-worker] ✓ ${message.type}: ${response.status}`);
      } else {
        // console.debug(`[debug][service-worker] ✓ ${message.type}:`, response);
      }
...
async function routeMessage(
  ...
  switch (type) {
    ...
    case 'ui_state_update':
      console.debug('[debug][service-worker] <- UI_STATE_UPDATE');
      return {
        success: true,
    ...
    case 'sidePanelOpened':
      ...
      console.debug('[debug][service-worker] <- SIDEPANEL_OPENED');
      return {
        success: true,
    ...
    case 'SUBTITLE_DATA':
      if (USE_ABORT_CONTROLLER_ARCHITECTURE) {
        ...
        console.debug('[debug][service-worker] → 收到字幕数据（V4架构会自动处理）');
        ...
        if (abortController) {
          ...
          console.debug(`[debug][service-worker] 字幕获取成功，已清除超时控制器`);
        }
        ...
        if (currentState === TranslateActiveState.PENDING) {
          console.debug('[debug][service-worker] 字幕捕获完成，继续执行翻译');
          // 添加tabId到data（如果没有的话）
    ...
    case 'translation_toggle':  // 支持新的消息类型
      ...
      if (USE_ABORT_CONTROLLER_ARCHITECTURE && abortTimeoutManagerInstance) {
        // 使用新架构
        console.debug('[debug][service-worker] 使用AbortController架构处理翻译');
    ...
    // 🆕 Hook XHR: 请求紧急翻译（目前仅作为通知，不需要处理）
    case 'REQUEST_URGENT_TRANSLATION':
      console.debug('[debug][service-worker][Hook] 收到紧急翻译请求通知:', data?.cacheKey);
      // 紧急翻译已经在handleToggleTranslateV4中处理
      // 这里只是接收Main World的通知，不需要额外操作
      return { success: true };
...
async function handleTogglePopup(sender: chrome.runtime.MessageSender, data?: any): Promise<any> {
  ...
  try {
    ...
    console.debug(`[debug][service-worker] 当前Popup状态: ${isCurrentlyOpen ? '已打开' : '已关闭'}`);
    ...
    if (isCurrentlyOpen) {
      ...
      await chrome.action.openPopup();
      console.debug(`[debug][service-worker] ✓ Popup已打开 (Tab:${tabId})`);
  ...
  try {
    ...
    try {
      ...
      if (localCache && localCache.availableSourceLanguages) {
        ...
        console.debug(`[debug][service-worker] ✓ 缓存命中[Local Storage]: ${availableSourceLanguages.length}个轨道`, {
...
async function handlePopupOpened(sender: chrome.runtime.MessageSender): Promise<any> {
  try {
    console.debug(`[debug][service-worker] <- popupOpened (已由Port机制处理状态)`);
...
async function handlePopupClosed(sender: chrome.runtime.MessageSender): Promise<any> {
  try {
    console.debug(`[debug][service-worker] <- popupClosed (已废弃，由Port机制处理)`);
...
async function handlePopupBlurred(sender: chrome.runtime.MessageSender): Promise<any> {
  try {
    console.debug(`[debug][service-worker] <- popupBlurred (已废弃，不再使用)`);
...
async function handleToggleSidePanel(sender: chrome.runtime.MessageSender, data?: any): Promise<any> {
  ...
  try {
    ...
    console.debug(`[debug][service-worker] SidePanel状态: enabled=${isCurrentlyEnabled}`);
    
    if (isCurrentlyEnabled) {
      // 当前已启用，执行关闭操作
      await chrome.sidePanel.setOptions({ tabId, enabled: false });
      console.debug(`[debug][service-worker] ✓ SidePanel已关闭 (Tab:${tabId})`);
      ...
      console.debug(`[debug][service-worker] ✓ SidePanel已打开 (Tab:${tabId})`);
...
async function fallbackToPopup(reason: string): Promise<void> {
  ...
  try {
    ...
    chrome.action.setPopup({ popup: 'src/popup/popup.html' }, async () => {
      ...
      try {
        await chrome.action.openPopup();
        console.debug('[debug][service-worker] ✓ Popup已打开');
      } catch (error) {
...
async function testSidePanelStateWithGetContexts(tabId: number): Promise<void> {
  try {
    console.debug(`[debug][service-worker] 测试getContexts检测 (Tab:${tabId})`);
    ...
    console.debug(`[debug][service-worker] SidePanel上下文数量: ${allSidePanelContexts.length}`);
    allSidePanelContexts.forEach((context, index) => {
      console.debug(`[debug][service-worker] SidePanel上下文 ${index}:`, {
    ...
    console.debug(`[debug][service-worker] 标签页${tabId}SidePanel上下文数量: ${tabSpecificContexts.length}`);
    
    // 方法3：对比getOptions结果
    const options = await chrome.sidePanel.getOptions({ tabId });
    console.debug(`[debug][service-worker] getOptions结果:`, {
    ...
    console.debug(`[debug][service-worker] 状态管理器状态: ${managerState}`);
    
    // 方法5：使用新的权威状态检测
    const authoritative = await getSidePanelState();
    console.debug(`[debug][service-worker] 权威状态检测: ${authoritative}`);
    
    // 总结对比
    const isOpenByGetContexts = tabSpecificContexts.length > 0;
    const isEnabledByGetOptions = options.enabled ?? false;
    console.debug(`[debug][service-worker] 状态对比总结:`, {
...
async function handleToggleSidePanelSync(sender: chrome.runtime.MessageSender, data?: any): Promise<any> {
  ...
  try {
    ...
    console.debug(`[debug][service-worker] getContexts检测: ${isActuallyOpen ? '已打开' : '未打开'} (上下文: ${sidePanelContexts.length})`);
    ...
    if (isActuallyOpen) {
      // 当前打开 → 关闭
      console.debug(`[debug][service-worker] 执行关闭操作`);
      await chrome.sidePanel.setOptions({ tabId, enabled: false });
      console.debug(`[debug][service-worker] ✓ SidePanel已关闭 (Tab:${tabId})`);
      ...
      // 当前关闭 → 打开
      console.debug(`[debug][service-worker] 执行打开操作`);
...
async function handleOpenSidePanel(sender: chrome.runtime.MessageSender, message?: any): Promise<any> {
  ...
  try {
    console.debug('[debug][service-worker] 设置sidepanel选项');
    ...
    console.debug(`[debug][service-worker] SidePanel已设置为启用 (标签页: ${tabId})`);
    ...
    if (source === 'user-action') {
      ...
      console.debug(`[debug][service-worker] 跨标签页同步操作 (${source})`);
    }
    
    // 🔧 关键修复：只有非跨标签页同步时才更新全局状态，避免无限循环
    if (source !== 'cross-tab-sync') {
      // 🔧 优化：移除状态保存，由Port连接处理
      console.debug(`[debug][service-worker] 状态保存由Port连接处理`);
      ...
      console.debug('[debug][service-worker] 跨标签页同步操作');
    }
...
async function handleOpenPopupFallback(sender: chrome.runtime.MessageSender): Promise<any> {
  try {
    console.debug('[debug][service-worker] popup降级策略');
...
async function handleGetAllState(data: any): Promise<any> {
  try {
    console.debug('[debug][service-worker] <- getAllState');
...
async function handleCheckTranslationCache(data: any): Promise<any> {
  try {
    console.debug('[debug][service-worker] 检查翻译缓存', {
      videoId: data.videoId,
    ...
    if (cachedData) {
      console.debug('[debug][service-worker] ✓ 找到翻译缓存');
      return {
        success: true,
        data: cachedData
      };
    } else {
      console.debug('[debug][service-worker] 翻译缓存未命中');
      return {
        success: false,
        data: null
      };
...
async function handleSubtitleData(data: any): Promise<any> {
  try {
    console.debug('[debug][service-worker] <- saveSubtitlesData:', {
      videoId: data.videoId,
      count: data.count,
      url: data.url
    });
    ...
    console.debug(`[debug][service-worker] ✓ 字幕数据已缓存: ${data.videoId}`);
...
async function handleToggleTranslate(sender: chrome.runtime.MessageSender, data: ToggleTranslateRequest): Promise<ToggleTranslateResponse> {
  try {
    ...
    console.debug('[debug][service-worker] 处理翻译切换参数:', { videoId, newState });
    ...
    // Step 1: 获取用户偏好配置
    console.debug('[debug][service-worker] Step 1: 获取用户偏好配置');
    let preferences;
    try {
      preferences = await userPreferencesManager.getUserPreferences();
      console.debug('[debug][service-worker] getUserPreferences 返回的数据:', {
    ...
    console.debug('[debug][service-worker] 用户偏好配置获取完成:', {
    ...
    // Step 2: 获取视频源语言数据
    console.debug('[debug][service-worker] Step 2: 获取视频源语言数据');
    ...
    if (sourceData && sourceData.availableSourceLanguages && sourceData.availableSourceLanguages.length > 0) {
      ...
      console.debug('[debug][service-worker] Step 2 调试 - sourceData.selectedSourceTrack:', {
    ...
    console.debug('[debug][service-worker] Step 3: 检查翻译结果缓存');
    ...
    console.debug('[debug][service-worker] 生成的缓存键:', cacheKey);
    ...
    if (cachedResult) {
      console.debug('[debug][service-worker] ✓ 找到缓存的翻译结果（P0级完全命中）');
    ...
    console.debug('[debug][service-worker] Step 4: 查找可复用的原始字幕');
    const partialCaches = await cacheManager.findByVideoAndSourceLang(videoId, sourceLang);
    
    if (partialCaches.length > 0) {
      console.debug(`[debug][service-worker] ✓ 找到${partialCaches.length}个相同源语言的缓存，复用原始字幕`);
      ...
      // 执行翻译
      console.debug('[debug][service-worker] 执行翻译（使用复用的原始字幕）');
    ...
    console.debug('[debug][service-worker] Step 5: 需要获取字幕数据');
    ...
    try {
      ...
      if (needFetchTracks) {
        console.debug('[debug][service-worker] Step 5.1: 获取轨道信息');
        ...
        try {
          ...
          if (trackResponse && trackResponse.success && trackResponse.tracks) {
            console.debug(`[debug][service-worker] 获取到${trackResponse.tracks.length}条轨道信息`);
            
            // Step 5.2: 选择最佳源语言
            if (sourceLang === 'auto') {
              // 调试：检查selectedSourceTrack的类型
              console.debug('[debug][service-worker] Step 5.2 调试 - sourceData?.selectedSourceTrack:', {
                languageCode: sourceData?.selectedSourceTrack?.languageCode,
            ...
            if (sourceLang && sourceLang !== 'auto') {
              try {
                console.debug(`[debug][service-worker] Step 5.3: 通过API设置字幕语言: ${sourceLang}` + (sourceKind ? ` (${sourceKind})` : ''));
      ...
      // Step 5.5: 触发字幕拦截器
      console.debug('[debug][service-worker] Step 5.5: 触发字幕拦截器');
      await chrome.tabs.sendMessage(tabId, {
...
async function continueTranslationWithSubtitles(data: any): Promise<any> {
  try {
    console.debug('[debug][service-worker] 继续翻译流程，处理字幕数据');
    ...
    // 记录源语言的来源
    if (passedSourceLang) {
      console.debug(`[debug][service-worker] 使用传递的源语言: ${passedSourceLang}`);
    } else if (sourceData?.selectedSourceTrack?.languageCode) {
      console.debug(`[debug][service-worker] 使用缓存的源语言: ${sourceData.selectedSourceTrack.languageCode}` +
    ...
    console.debug('[debug][service-worker] 开始翻译字幕:', {
      videoId,
      sourceLang,
      targetLang: preferences.targetLang,
    ...
    // 调试：打印完整的preferences
    console.debug('[debug] preferences内容:', {
      targetLang: preferences.targetLang,
      translationService: preferences.translationService,
...
async function executeTranslation(subtitleData: SubtitleData, preferences: UserPreferences): Promise<any> {
  try {
    console.debug('[debug][service-worker] 执行两阶段翻译（基于时间间隔断句）:', {
    ...
    console.debug('[debug][service-worker] 检测源语言:', sourceLang);
    ...
    const onProgress = (phase: string, progress: number, data?: any) => {
      ...
      if (phase === 'urgent' && data) {
        ...
        console.debug(`[debug][service-worker] 紧急翻译完成: ${data.translatedCount}条，耗时${data.timeElapsed}ms`);
        ...
        // 批量翻译进度
        console.debug(`[debug][service-worker] 批量翻译进度: ${progress.toFixed(0)}% (批次${data.currentBatch}/${data.totalBatches})`);
    ...
    console.debug(`[debug][service-worker] ✓ 渐进式翻译完成: ${translatedSubtitles.length}条字幕`);
...
function detectSourceLanguage(subtitles: any[]): string {
  console.debug('[debug] detectSourceLanguage开始检测，字幕数量:', subtitles?.length);
  
  // 简单的语言检测逻辑
  // 可以根据字幕文本的字符特征判断语言
  if (!subtitles || subtitles.length === 0) {
    console.debug('[debug] 无字幕，返回默认语言: en');
    return 'en'; // 默认英语
  }
  
  // 取前几条字幕进行检测
  const sampleTexts = subtitles.slice(0, 5).map(s => s.text).join(' ');
  console.debug('[debug] 用于检测的样本文本:', sampleTexts.substring(0, 100));
  
  // 检测是否包含中文字符
  if (/[\u4e00-\u9fa5]/.test(sampleTexts)) {
    console.debug('[debug] 检测到中文字符，返回: zh');
    return 'zh';
  }
  
  // 检测是否包含日文字符
  if (/[\u3040-\u309f\u30a0-\u30ff]/.test(sampleTexts)) {
    console.debug('[debug] 检测到日文字符，返回: ja');
    return 'ja';
  }
  
  // 检测是否包含韩文字符
  if (/[\uac00-\ud7af]/.test(sampleTexts)) {
    console.debug('[debug] 检测到韩文字符，返回: ko');
    return 'ko';
  }
  
  // 默认返回英语
  console.debug('[debug] 未检测到特殊字符，默认返回: en');
  return 'en';
}
...
async function translateBatch(
  ...
  console.debug('[debug] translateBatch接收参数:', {
...
async function translateWithGoogle(
  ...
  console.debug(`[debug] ${translationType}请求:`, {
  ...
  if (lines.length > 0) {
    ...
    const logInfo = lines.length > 1 
      ...
      : `[单条: "${firstLine}"]`;
    console.debug(`[debug] ${translationType}原文 (${lines.length}条): ${logInfo}`);
  }
  ...
  for (const endpoint of endpoints) {
    try {
      ...
      console.debug(`[debug][service-worker] Google endpoint=${endpoint.id} success`);
      return translatedTexts;
    } catch (error) {
...
async function handleSidePanelActuallyOpened(message: any): Promise<any> {
  try {
    console.debug('[debug][service-worker] 收到SidePanel实际打开通知:', message);
    console.debug('[debug][service-worker] 状态保存已由Port连接处理');
...
async function handleSidePanelActuallyClosed(message: any): Promise<any> {
  try {
    console.debug('[debug][service-worker] 收到SidePanel实际关闭通知:', message);
    ...
    // 确保运行时状态为关闭
    await runtimeStateManager.setPopupState(false);
    console.debug('[debug][service-worker] ✓ SidePanel状态已确认为关闭');
...
async function handleCheckSidePanelStatus(sender: chrome.runtime.MessageSender): Promise<any> {
  try {
    ...
    try {
      ...
      console.debug(`[debug][service-worker] 标签页${tabId} SidePanel状态: enabled=${isEnabled}`);
...
async function handleGetSidePanelStatus(_sender: chrome.runtime.MessageSender): Promise<any> {
  try {
    ...
    console.debug(`[debug][service-worker] getSidePanelStatus: enabled=${isEnabled}`);
...
async function testPaidApiService(apiType: string, apiKey: string, model?: string, tier?: 'free' | 'pro'): Promise<{success: boolean, message: string}> {
  ...
  try {
    if (apiType === 'openai') {
      ...
      return await testQwenService(apiKey);
    } else {
      return {
        success: false,
...
async function testGoogleTranslateService(testText: string, sourceLang: string, targetLang: string): Promise<string> {
  ...
  try {
    console.debug('[debug][service-worker] 测试Google翻译路径A');
  ...
  // 测试路径B: /translate_a/t
  try {
    console.debug('[debug][service-worker] 测试Google翻译路径B');
...
async function testMicrosoftTranslateService(testText: string, sourceLang: string, targetLang: string): Promise<string> {
  ...
  // 测试路径A: Edge认证令牌
  try {
    console.debug('[debug][service-worker] 测试微软翻译路径A');
  ...
  // 测试路径B: API-Edge端点
  try {
    console.debug('[debug][service-worker] 测试微软翻译路径B');
...
async function testDeepLService(apiKey: string, tier?: 'free' | 'pro'): Promise<{success: boolean, message: string}> {
  ...
  const options = {
    ...
    body: JSON.stringify({
      text: ['Hello'],
      target_lang: 'ZH-HANS'  // 翻译成简体中文
    })
  };

  try {
    console.log('[service-worker] 测试DeepL API:', { endpoint, tier: isFreeKey ? 'free' : 'pro' });
    ...
    if (!response.ok) {
      ...
      switch (response.status) {
        ...
        case 529:
          ...
          break;
        default:
          errorMsg = errorData?.message || `HTTP ${response.status}: ${response.statusText}`;
      }
      throw new Error(errorMsg);
    }

    const data = await response.json();
    console.log('[service-worker] DeepL API响应:', JSON.stringify(data, null, 2));

    // 检查响应结构
    if (!data.translations || !Array.isArray(data.translations)) {
      throw new Error('DeepL返回格式异常: 缺少translations字段');
    }

    if (data.translations.length === 0) {
      throw new Error('DeepL返回格式异常: translations数组为空');
    }

    const result = data.translations[0].text || '测试成功';
    const detectedLang = data.translations[0].detected_source_language || 'EN';

    console.log('[service-worker] ✓ DeepL测试成功:', result);

    return {
      success: true,
      message: `DeepL API测试成功 (${isFreeKey ? '免费层' : '付费层'})\n翻译结果: ${result}\n检测语言: ${detectedLang}`
    };
  } catch (error) {
 ...
 * 测试Qwen API服务
 */
async function testQwenService(apiKey: string): Promise<{success: boolean, message: string}> {
  ...
  try {
    console.log('[service-worker] 测试Qwen API:', { endpoint, model: 'qwen-mt-plus' });
    const response = await fetch(endpoint, options);
    ...
    if (!response.ok) {
      ...
      switch (response.status) {
        ...
        case 429:
          ...
          break;
        case 400:
          errorMsg = errorData?.error?.message || 'API请求参数错误';
          break;
        case 500:
        case 502:
        case 503:
          errorMsg = 'Qwen API服务器错误，请稍后重试';
          break;
        default:
          errorMsg = errorData?.error?.message || `HTTP ${response.status}: ${response.statusText}`;
      }
      throw new Error(errorMsg);
    }

    const data = await response.json();
    console.log('[service-worker] Qwen API响应:', JSON.stringify(data, null, 2));

    // 检查响应结构
    if (!data.choices || !Array.isArray(data.choices)) {
      throw new Error('Qwen返回格式异常: 缺少choices字段');
    }

    if (data.choices.length === 0) {
      throw new Error('Qwen返回格式异常: choices数组为空');
    }

    const result = data.choices[0]?.message?.content || '测试成功';

    console.log('[service-worker] ✓ Qwen测试成功:', result);

    return {
      success: true,
      message: `Qwen API测试成功\n模型: qwen-mt-plus\n翻译结果: ${result}`
    };
  } catch (error) {
