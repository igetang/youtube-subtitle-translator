...
export class TwoPhaseTranslatorV4 {
  ...
  private static readonly BATCH_START_DELAY = 200;  // 批量翻译启动延迟（降低到200ms）
  ...
  public async translateUrgent(
    ...
    try {
      ...
      if (currentIndex === -1) {
        console.log('[TwoPhaseTranslatorV4] 未找到当前播放位置，使用第一条字幕');
      }
  ...
  public async translateBatch(
    ...
    try {
      ...
      // 获取已翻译的索引
      const urgentIndexes = new Set(urgentResults.map(r => r.index));
      
      // 准备批量数据（排除已紧急翻译的）
      const batchSubtitles = subtitles.filter((_, idx) => !urgentIndexes.has(idx));
      
      if (batchSubtitles.length === 0) {
        console.log('[TwoPhaseTranslatorV4] 所有字幕已被紧急翻译覆盖');
        return results;
      }
      
      // 使用智能分段
      const timeGaps = TimeGapAnalyzer.analyzeGaps(batchSubtitles);
      const batches = this.segmenter.segmentByTimeGaps(batchSubtitles, timeGaps);
      ...
      for (let i = 0; i < batches.length; i++) {
        ...
        try {
          ...
          console.log(`[TwoPhaseTranslatorV4] ✓ 批次 ${i + 1}/${batches.length} 完成`);
          ...
          // 批次失败，使用原文
          console.warn(`[TwoPhaseTranslatorV4] 批次 ${i + 1} 翻译失败，使用原文`);
          batch.forEach(sub => {
            const originalIndex = subtitles.indexOf(sub);
            if (originalIndex !== -1) {
              results.push({
    ...
    return new Promise(async (resolve, reject) => {
      ...
      try {
        ...
        if (service.type === 'openai') {
          ...
          console.log('[TwoPhaseTranslatorV4] 前3个文本:', texts.slice(0, 3));
