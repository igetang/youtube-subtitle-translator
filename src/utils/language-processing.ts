...
export function findMatchingTargetLanguage(codeToMatch: string): Language | undefined {
    ...
    console.log(`[language-processing] 尝试匹配语言代码: ${codeToMatch}`);
    ...
    if (matchedLang) {
        console.log(`[language-processing] P3 匹配(列表通用->输入特定): 输入'${normalizedCodeToMatch}'匹配列表项'${matchedLang.code}'`);
        return matchedLang;
    }
    ...
    if (matchedLang && baseLang === matchedLang.code.toLowerCase()) { // Ensure it's a base match
         console.log(`[language-processing] P4 匹配(输入特定->列表通用): 输入'${normalizedCodeToMatch}'匹配列表项'${matchedLang.code}'(基础匹配)`);
         return matchedLang;
    }
    ...
    if (baseLang === 'zh' && !matchedLang) {
        console.log('[language-processing] P2 "zh"的备选方案: 默认使用zh-Hans(如果可用),然后是zh-Hant.');
    ...
    console.log(`[language-processing] 经过所有检查后未找到${codeToMatch}的合适匹配.`);
    return undefined;
} 