...
YouTube字幕翻译Chrome扩展在多标签页环境下存在状态同步问题，影响用户体验的一致性。

## 🚨 当前存在的问题

### 问题1: SidePanel实例不跨标签同步开启 🟡
**状态**: 设计限制，需要产品决策

**问题描述**:
- Tab A开启SidePanel后，切换到Tab B
- Tab B的翻译设置按钮状态正确显示为"开启"
- 但Tab B的SidePanel面板本身并未自动打开
- 用户需要再次点击按钮才能打开SidePanel

**技术分析**:
- 这是Chrome SidePanel API的设计特性
- 每个标签页的SidePanel实例是独立的
- `chrome.sidePanel.open()`只影响当前标签页

**可能的解决方案**:
1. **主动同步方案**: 检测到标签切换时自动打开目标标签的SidePanel
   - ✅ 优点: 用户体验一致
   - ❌ 缺点: 可能让用户觉得突兀，侵入性强
2. **被动指示方案**: 仅同步按钮状态，SidePanel按需打开
   - ✅ 优点: 尊重用户意图，非侵入性
   - ❌ 缺点: 存在状态认知差异
3. **混合方案**: 提供用户配置选项
...
### 问题2: 手动关闭SidePanel后按钮状态不更新 🔴
**状态**: 技术挑战，寻求可行方案

**问题描述**:
- 用户点击SidePanel右上角X按钮关闭面板
- 翻译设置按钮状态仍显示为"开启"
- 其他标签页的按钮状态也未同步更新
- 导致UI状态与实际状态不一致
...
window.addEventListener('beforeunload', () => {
  chrome.runtime.sendMessage({action: 'sidePanelActuallyClosed'});
});
```
...
3. **Chrome API缺失**: 没有专门的SidePanel关闭事件API
...
### 三层同步机制
```
SidePanel生命周期 → Background状态管理 → 跨标签UI同步
      ↓                    ↓                    ↓
生命周期通知          Session存储           UI状态更新
```

**数据流**:
1. Tab A打开设置 → SidePanel打开 → 发送`sidePanelActuallyOpened`
...
- 维护全局SidePanel状态(session storage)
- 协调跨标签消息传递

**SidePanel** (生命周期通知):
- DOMContentLoaded时发送`sidePanelActuallyOpened`
- beforeunload时发送`sidePanelActuallyClosed`(有限制)
