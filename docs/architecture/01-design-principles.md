...
> **最后更新**: 2025-10-25  
> **版本**: v4.0.0  
> **当前方案**: ✅ **Popup + AbortController V4 两阶段翻译架构**（生产环境运行中）

## 🚨 **方案变更记录**

### **✅ 当前采用方案摘要**
- **Popup 首选 + Fallback**：`UIManager` 优先通过 `chrome.action.openPopup()` 打开设置界面，失败时回退到背景消息（`openPopupFallback`）。
- **AbortController V4 会话管理**：`handle-toggle-translate-v4.ts` / `AbortTimeoutManager` 使用组合信号（超时 + 手动取消）驱动两阶段翻译、缓存复用与错误提示。
- **两阶段翻译器**：`TwoPhaseTranslatorV4` 负责紧急字幕（300ms内显示）与批量字幕全量覆盖，动态适配各翻译服务配置。
- **MessageBus 单例**：`src/shared/messages/message-bus.ts` 提供跨进程消息路由，所有子系统通过 type 字段通信。

### **🗂️ 历史方案：SidePanel（已归档）**
- **弃用原因**：Chrome 版本兼容性差、权限复杂、用户体验不一致、跨标签同步难维护。
- **保留资料**：完整技术文档与源代码归档于 [/docs/archive/deprecated-sidepanel/](../archive/deprecated-sidepanel/) 供回溯参考。
...
**核心思想**：**用户体验优先 + 技术实现简化 + 可观测性可控**
- **Popup Fallback**：所有页面点击扩展图标都能响应；若 `chrome.action.openPopup` 不可用则 fallback。
- **两阶段翻译**：紧急 + 批量流程，结合缓存与日志提示避免用户无感错位。
- **消息驱动架构**：MessageBus 单例负责前台/后台/main-world 通信。
- **会话化执行**：AbortController V4 管控翻译生命周期，确保状态可预测。
- **组件化职责**：后台组件、内容脚本、共享模块各司其职，通过消息协作。
...
### Popup Fallback 与 V4 会话架构（2025-07 ~ 2025-10） ⭐

**Popup Fallback（2025-07）**
- 解决 SidePanel 在多版本 Chrome 下“死按钮”问题。
- 页面检测 + 双界面策略：YouTube 显示设置，非 YouTube 显示使用说明。
- 移除 `scripting` 权限依赖，简化权限模型。

**AbortController V4 + 两阶段翻译（2025-09 ~ 2025-10）**
- `AbortTimeoutManager` / `TranslationSession` 统一翻译会话生命周期。
- 紧急阶段失败会显示警告继续批量；批量阶段任何批次失败直接抛错并回退。
- 缓存写入改用 VTT 字符串，`findByVideoAndSourceLang` 复用原字幕，避免重复抓取。
- 日志规范升级：关键步骤 log，细节 debug，错误 warn/error。
...
```typescript
// UserPreferences配置获取
{ type: 'getUserPreferences', data?: any }
