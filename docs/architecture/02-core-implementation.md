...
#### 5.16.11 2025-09-26 轨道切换校正

- `ToggleTranslateRequest` 新增 `sourceLang` 字段，content-script 在检测到源语言变更时会把用户选择的语言随重新翻译请求一并发送。
- Service Worker 的 Stage 2/3 会优先使用这个字段匹配缓存中的轨道，并且 *无论轨道缓存是否命中*，都会调用 `setSubtitleTrackAPI` 强制切换 YouTube 控制栏的字幕轨。
- 切轨成功后再触发字幕按钮，从而保证拦截器抓取的始终是最新源语言的字幕。
- 架构目标回归到“存储监听 → 重新翻译 → 先切轨再抓字幕”的设计，避免只依赖缓存语言导致依旧抓到旧轨道。
