...
> ⚠️ **此迁移已于2025年1月完成**
> **注：MESSAGE_FORMAT_MIGRATION.md的内容已合并到本文档**
...
## 📋 消息格式统一（从MESSAGE_FORMAT_MIGRATION.md合并）
...
#### 完成的修改：
- ✅ Background Service Worker: 消息路由从action改为type
- ✅ UI Manager: 所有sendMessage调用使用type字段
- ✅ Content Script: 消息格式统一为type + data结构
- ✅ Popup: 消息检查增加向后兼容性

#### 涉及的消息类型：
- `getRuntimeState` - 获取运行时状态
- `setRuntimeState` - 设置运行时状态  
- `openPopup` - 打开弹窗（替代openSidePanel）
- `UI_STATE_UPDATE` - UI状态更新通知
- `POPUP_STATE_CHANGED` - 弹窗状态变化（替代SIDEPANEL_STATE_CHANGED）
