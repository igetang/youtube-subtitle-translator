    ...
    return new Promise((resolve) => {
      chrome.storage.local.get(['sourceLang', 'targetLang', 'translationApi'], (result) => {
        resolve({
          ...
          translationApi: result.translationApi || 'google-free'
        });
      });
    });
  }
    ...
    if (data.source === 'main-world' && data.type === 'CAPTION_TRACKS_RESPONSE') {
      ...
      if (data.error) {
        ...
        chrome.runtime.sendMessage({
          ...
          error: data.error,
          _requestId: data._requestId  // 🔧 新增：携带requestId
        });
      ...
      // C32-C39: 按照架构文档处理字幕轨道响应
      const videoId = getVideoId();
      if (!videoId) {
        console.error('[content-script] 无法获取视频ID，无法处理字幕轨道');
        return;
      }
      
      console.log(`[content-script] C32: 从Main World memory cache获取到${captionTracks.length}条字幕轨道信息`);
      ...
      console.log('[content-script] C33: 发送saveTrackCache到Background (保存到Memory Cache)');
      LocalStorageProxy.getInstance().saveTrackMemoryCache(videoId, processedTracks)
        .then((result: any) => {
          if (result.success) {
            console.log('[content-script] C34: Background轨道Memory Cache保存成功 ⚡');
            
            // C35: ContentScript: startTranslationProcess
            console.log('[content-script] C35: 开始翻译处理流程');
            handleTranslationProcessAfterTracks(captionTracks, videoId);
            
          } else {
            console.warn('[content-script] 轨道Memory Cache保存失败:', result.error);
            // 即使Memory Cache失败也继续翻译流程
            handleTranslationProcessAfterTracks(captionTracks, videoId);
          }
        })
        .catch((error: any) => {
          console.error('[content-script] 保存轨道Memory Cache出错:', error);
          // 即使Memory Cache失败也继续翻译流程
          handleTranslationProcessAfterTracks(captionTracks, videoId);
        });
...
function initializeUIManager() {
  ...
  // ✅ 移除翻译状态事件监听器（现在使用按钮直接调用模式）
  
  // C10: 监听翻译开始请求，按照架构文档流程处理
  eventBus.on('translation:start_requested', (data) => {
    console.log('[content-script] 收到翻译开始请求');
    handleTranslationStartRequest(data);
  });
  
  // C40: 监听翻译停止请求
  eventBus.on('translation:stop_requested', (data) => {
    console.log('[content-script] 收到翻译停止请求');
    handleTranslationStopRequest(data);
  });
...
function initializeControlPanel() {
  ...
  try {
    ...
    // ✅ 移除旧的事件监听器，现在使用统一的local storage管理流程
    // 翻译流程现在通过handleTranslationStartRequest -> Background local storage检查 -> 轨道处理来实现
    
          console.log('[content-script] 翻译控制面板初始化完成（使用新流程）');
  } catch (error) {
...
function handleTranslationStopRequest(data: any): void {
  ...
  // 停止相关的翻译处理
  eventBus.emit('translation:stopped', {
...
async function startTranslationProcess(captionTracks: any[], videoId: string): Promise<void> {
  ...
  try {
    ...
    // ✅ 通过LocalStorageProxy获取翻译设置
    const settings = await LocalStorageProxy.getInstance().getTranslationSettings();
    const sourceLang = settings.sourceLang;
    const targetLang = settings.targetLang;
    const translationApi = settings.translationApi;
    
    console.log(`[content-script] 使用设置: 源语言=${sourceLang}, 目标语言=${targetLang}, API=${translationApi}`);
    ...
    console.log(`[content-script] 已找到源语言轨道: ${sourceTrack.languageCode}`);
    ...
    console.log(`[content-script] 已获取${sourceSubtitles.length}条源字幕`);
    
    if (needsTranslation) {
      // 需要翻译的情况
      console.log('[content-script] 未找到原生目标轨道，启动翻译...');
      ...
      translationDispatcher.startProgressiveTranslation(context, (processedEvents) => {
        // 翻译回调处理
        console.log(`[content-script] 翻译完成，收到${processedEvents.length}条处理后字幕`);
        
        // C37: ContentScript发送: saveTranslationCache
        console.log('[content-script] C37: 保存翻译结果到local storage');
        ...
        // 将处理后的字幕转换为local storage格式
        const cacheData = {};
        processedEvents.forEach((event: any) => {
          if (event.targetText) {
            (cacheData as any)[event.id || `sub-${Math.random()}`] = event.targetText;
          }
        });
        
        console.log(`[content-script] 翻译数据格式转换完成: ${Object.keys(cacheData).length}条记录准备保存到cache`);
        
        // C37-C38: 保存翻译结果到Background local storage
        if (Object.keys(cacheData).length > 0) {
          LocalStorageProxy.getInstance().saveTranslationLocalStorage(videoId, translationParams, cacheData)
            .then((result) => {
              if (result.success) {
                console.log(`[content-script] C38: Background翻译cache保存成功: 缓存键=subtitle_translation_cache_${videoId}_${translationParams.targetLang}_${translationParams.apiType}`);
              } else {
                console.warn('[content-script] 翻译local storage保存失败:', result.error);
              }
            })
            .catch((error) => {
              console.warn('[content-script] 保存翻译local storage时出错:', error);
            });
        } else {
          console.warn('[content-script] 没有有效的翻译数据需要保存到cache');
        }
        
        // C39: ContentScript: 显示翻译字幕
        console.log('[content-script] C39: 显示翻译后的字幕');
        displayTranslatedSubtitles(processedEvents);
      });
    } else {
      // 有原生目标轨道的情况
      console.log('[content-script] 找到原生目标轨道，获取目标字幕数据...');
    ...
    console.error('[content-script] 翻译处理出错:', error);
    // 发送翻译错误事件
    eventBus.emit(EventTypes.TRANSLATION_ERROR, {
...
async function fetchSubtitleData(baseUrl: string): Promise<any[]> {
  try {
    ...
    console.error('[content-script] 获取字幕数据出错:', error);
    throw error;
  }
}
...
function parseSubtitleData(subtitleText: string): any[] {
  ...
  try {
    ...
    console.error('[content-script] 解析字幕数据出错:', error);
    return [];
  }
}
...
/**
 * 显示翻译后的字幕
 * @param subtitles 处理后的字幕事件或local storage数据格式 { [id: string]: string }
 */
function displayTranslatedSubtitles(subtitles: any[] | any): void {
  // 检查是否是local storage数据格式 (object with id->text mapping)
  if (subtitles && typeof subtitles === 'object' && !Array.isArray(subtitles)) {
    console.log(`[content-script] 检测到local storage数据格式，转换为数组格式`);
    // 将local storage格式转换为数组格式（这里需要时间信息，暂时使用占位符）
    const subtitleArray = Object.entries(subtitles).map(([id, text], index) => ({
      id,
      targetText: text,
      sourceText: '', // local storage中没有源文本信息
      start: index * 5, // 临时时间值，实际应用中需要更复杂的处理
      end: (index + 1) * 5
    }));
    
    console.log(`[content-script] 从local storage转换后显示${subtitleArray.length}条字幕`);
    subtitles = subtitleArray;
  } else if (Array.isArray(subtitles)) {
    console.log(`[content-script] 数组格式，开始显示${subtitles.length}条字幕`);
  } else {
    console.error('[content-script] 未知的字幕数据格式:', typeof subtitles);
    return;
  }
  ...
  if (!subtitleOverlay) {
    console.log('[content-script] 未找到字幕叠加层，等待创建');
    // 无需自行创建，由UI管理器负责创建容器
  ...
  if (!videoElement) {
    console.error('[content-script] 无法找到视频元素');
    return;
  }
