...
function setupMessageHandlers() {
  ...
  chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
    ...
    if (messageType === 'GET_SUBTITLE_DATA') {
      console.log('[content-script] 收到GET_SUBTITLE_DATA请求:', message.data);
      ...
      const handleRequest = new Promise((resolve) => {
        ...
        const handleSubtitleResponse = (event: MessageEvent) => {
          if (event.data && event.data.source === 'main-world' && 
            ...
            if (!responded) {
              responded = true;
              console.log('[content-script] 收到字幕数据响应:', {
                hasSubtitles: !!event.data.subtitles,
                subtitleCount: event.data.subtitles?.length || 0,
        ...
        // 向主世界脚本请求字幕数据
        console.log('[content-script] 向main-world发送字幕请求，requestId:', requestId);
        window.postMessage({
          source: 'content-script',
        ...
        setTimeout(() => {
          ...
          if (!responded) {
            responded = true;
            console.warn('[content-script] 字幕请求超时，requestId:', requestId);
            resolve({
              success: false,
              error: '获取字幕超时（4秒）'
            });
      ...
      // 异步发送响应 - 确保Promise解析后才发送
      handleRequest
        .then((result) => {
          console.log('[content-script] 准备发送字幕响应给Service Worker:', result.success ? '成功' : '失败');
          sendResponse(result);
        })
        .catch((error) => {
          console.error('[content-script] 处理字幕请求异常:', error);
          sendResponse({
            success: false,
            error: error.message || '处理字幕请求失败'
          });
        });
    ...
    if (messageType === 'REQUEST_SUBTITLE_CAPTURE') {
      console.log('[content-script] 收到字幕捕获请求');
  ...
  try {
    ...
    if (configResponse.hasCache && configResponse.config.sourceLang && configResponse.config.targetLang) {
      ...
      if (cacheCheckResponse.success && cacheCheckResponse.hasCache) {
        ...
        console.log('[content-script] C27: 翻译结果缓存未完全匹配，直接调用API获取字幕轨道');
        // 直接跳到O: 调用API获取字幕轨道
      }
    } else {
