/**
 * @file settings-manager.legacy.ts
 * @description This file is intentionally left blank as it is being deprecated.
 * All functionalities have been migrated to user-preferences-manager.ts.
 */

import { StorageManager, StorageKeys, StorageArea } from './storage-manager';
import { SubtitleMode } from '../types/user-preferences-types';  // 统一导入
...
/**
 * 用户设置
 * @deprecated 此接口即将废弃，请使用 UserPreferences 代替
 * @see src/shared/types/user-preferences-types.ts
 * 
 * 迁移指引：
 * - 使用 UserPreferencesManager 替代 SettingsManager
 * - 使用 convertUserSettingsToUserPreferences() 进行数据转换
 * - sourceLang 字段已移除，改由 VideoSourceLanguageCache 管理
 * 
 * ⚠️ 注意：translateActive 已从此接口移除
 * 翻译状态现由 RuntimeStateManager 管理，详见 architecture.md 7.1.2
 */
export interface UserSettings {
  ...
  // translateActive: boolean;  // 🚫 已移除：现由 RuntimeStateManager 管理
...
export const DEFAULT_SETTINGS: UserSettings = {
  ...
  // translateActive: false,  // 🚫 已移除：现由 RuntimeStateManager 管理
  ...
  private triggerChangeEvent(
    ...
    switch (settingKey) {
      ...
      case 'subtitleMode':
        ...
        break;
      // case 'translateActive':  // 🚫 已移除：现由 RuntimeStateManager 管理
